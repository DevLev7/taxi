	<section id="<?php echo $building_row; ?>" class="dialogs">
		<div class="container-fluid">
			<div class="row">
				<div class="col-s">
					...
					<?php
					$dialogs_query=get_sub_field('dialog-posts');
					$arr_posts = [];
					$arr_k = -1;
					...
					foreach ($dialogs_query as $key1 => $post_id) {
						...
						$arr_posts[$arr_k]['question']['text'] = get_the_title( $post_id);
					}
					
					$arr=get_sub_field('dialog-repeater');
					$arr_date = [];
					$k = 0;
					
					if(!empty($dialogs_query) && !empty($arr)){
						echo "Диалоги пусты";
					}elseif(empty($dialogs_query) && !empty($arr)){						
						$arr = $arr;
					}elseif(!empty($dialogs_query) && empty($arr)){						
						$arr = $arr_posts;
					}else{
						$arr = array_merge($arr_posts, $arr);
					}
					...
					foreach ($arr as $key => $value) {
						...
						$length_question_text = mb_strlen($value['question']['text'],'UTF-8'); 
						...
						$arr_date[$k] = [
							...
							"question-voite" => round((($date_generate_day*$date_new_m*$length_question_text)/(680))+$date_new_m),
							"answer-voite" => round((($date_new_m*($length_answer_text-$length_question_text))/(75))),
						];
					}
